...
export const useLabelSeg = (
  ...
  const createPolygon = (options: {
    points: { x: number; y: number }[]
    data?: { labelId: string; classId: number }
  }) => {
  ...
  const continueDrawing = (e: MouseEvent) => {
    ...
    if (points.value.length > 0) {
      currentPolygon.value?.set('points', [
        ...points.value,
  ...
  const finishDrawing = async () => {
    ...
    try {
      ...
      if (
        data?.upsertLabelSegmentations.__typename ===
        'UpsertLabelSegmentationSuccess'
      ) {
  ...
  const addExistingLabel = async (
    label: LabelSegmentation
  ): Promise<CustomPolygon | null> => {
  ...
  const handleModification = async (
    ...
    try {
      ...
      if (
        result.data?.upsertLabelSegmentations.__typename ===
        'UpsertLabelSegmentationSuccess'
      ) {
  ...
  const handleKeyDown = async (e: KeyboardEvent) => {
    ...
    if (e.key === 'Escape' && isDrawing.value) {
      ...
      points.value = []
      if (currentPolygon.value) {
        fabricCanvas.value.remove(
          currentPolygon.value as unknown as FabricObject
        )
        currentPolygon.value = null
        fabricCanvas.value.renderAll()
      }
